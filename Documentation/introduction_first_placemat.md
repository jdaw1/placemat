...
It is a PostScript program, and soes not hide that. 
It consists of a single long text file, with user-chosen paramneters at the top, and Postript code underneath. 
Those user-chosen paramneters are written in Postscript, in PostScript syntax. 
Placemats can be made by those with no prograaming expertise. 
...
Yes, it can be done in a few minutes &mdash; but not your first placemats.
...
Bookmark this manual, and download to your machine a copy of the PostScript code (update this copy every time: new versions fix bugs; add features; improve default settings). 
Store it in a directory named something like “placemats”. 
Make a copy of the original PostScript. 
Give it a name which will make sense in aeons to come, perhaps YYYYMMDD.ps, or YYYYMMDD_TastingName.ps (think of it as your the first item in your placemat archive).

### Converting the .ps to PDF

You have saved a .ps file. 
This file is to be opened in two different ways: with the code editor; and with a program to convert it from .ps to .pdf. Let’s master the latter. 
...
<img align="right" width="549" height="797" src="images/placemat.png">
...
For a sense of which of the many features might be wanted look at the [list of old placemats](file:///Users/JDAW/Documents/HTML/www.jdawiseman.com/papers/placemat/placemats_list.html), some of which have helpful comments and advice.
...
They should be short, ideally 2 or 3 characters, certainly &le;5 characters. 
...
Each of the items of the array &mdash; this configuration having five &mdash; is a string, so delimited with round brackets, `()`. 
...
In each circle, below the `Titles`, are the unimaginatively named `Belowtitles`. 
...
For a first placemat perhaps restrict yourself to plain strings.
...
```
By copy-paste move the four changed parameters to above that line. 
...
Observe that these parameters have a value that is a name, so beginning with a `/`. 
...
If it isn&rsquo;t a ThePortForum event, remove the ThePortForum logo logos by setting `ThePortForumIconPlacement` and `ThePortForumIconTastingNotePlacement` to `/None` (again, a name-type parameter).
...
If the tasting is being arranged via a wine bulletin board, upload the PDF to the web, and post in an appropriate thread a link labelled something like “Current draft of the placemats”. Allow some time for comments and corrections and changes. Update as appropriate.
...
But for many possible non-huge tastings in a single-session, the above allows most of what is wanted.
